...
public sealed class {{CLASS_NAME}} : IFractalCalculator, IDisposable
{
    ...
    private const double Bailout2 = {{BAILOUT_RADIUS_SQ}};
    ...
    private const bool SaEnabled = {{SA_Z2C_ENABLED}};
    ...
    private const bool SupportsPerturbation = {{SUPPORTS_PERTURBATION}};
    ...
    private const bool SupportsDd4Direct = {{SUPPORTS_DD4_DIRECT}};
    ...
    private const bool SupportsDe = {{SUPPORTS_DE}};
...
{{PREV_DECL_SCALAR}}        int it = 0;
        int maxIt = MaxIterations;
        for (; it < maxIt; it++)
        {
            double r2 = zr * zr + zi * zi;
            if (r2 >= Bailout2) break;
{{ITER_DECL_SCALAR}}{{SCALAR_D_BODY}}
{{SCALAR_Z_BODY}}
{{PREV_UPDATE_SCALAR}}            zr = zr_new; zi = zi_new;
            dr = dr_new; di = di_new;
        }
...
{{PREV_DECL_AVX2}}        Vector256<double> bailoutVec = Vector256.Create(Bailout2);
...
{{BLA_B_BODY}}
                    ...
                    {
{{ITER_DECL_QD_REF}}{{QD_Z_BODY}}
{{PREV_UPDATE_QD_REF}}                        zr_q = zr_q_new; zi_q = zi_q_new;
                    }
                }
...
{{PERTURB_DELTA_BODY}}
    ...
    {
        QD zr_q = QD.Zero, zi_q = QD.Zero;
{{PREV_DECL_QD_REF}}        for (int n = 0; n < maxIt; n++)
        {
            double zrHi = zr_q.X0, ziHi = zi_q.X0;
            if (zrHi * zrHi + ziHi * ziHi >= Bailout2) return n;
            {
...
{{PREV_UPDATE_QD_REF}}                zr_q = zr_q_new; zi_q = zi_q_new;
    ...
    private uint ComputePixelDdDirect(DD cr_dd, DD ci_dd, int maxIt, int bufIdx = -1)
    {
        ...
        double drv = 0.0, div = 0.0;
{{PREV_DECL_DD_DIRECT}}        DD finalZMag2 = DD.Zero;
        double finalZr = 0.0, finalZi = 0.0;
        int it = 0;
        for (; it < maxIt; it++)
        {
...
{{ITER_DECL_DD_DIRECT}}{{DD_DIRECT_BODY}}
{{PREV_UPDATE_DD_DIRECT}}            zr_dd = zr_dd_new; zi_dd = zi_dd_new;
        }
...
{{PREV_DECL_QD_DIRECT}}        DD finalZMag2 = DD.Zero;
        double finalZr = 0.0, finalZi = 0.0;
        int it = itStart;
        for (; it < maxIt; it++)
        {
...
{{ITER_DECL_QD_DIRECT}}{{QD_DIRECT_BODY}}
{{PREV_UPDATE_QD_DIRECT}}            zr_q = zr_q_new; zi_q = zi_q_new;
        }
        return ColorForDd(it, finalZr, finalZi, finalZMag2, drv, div, maxIt, bufIdx);
    }
