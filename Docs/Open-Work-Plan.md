...
| SM-11 | Deep-zoom render is reference-dependent → nav "close but not exact" / pan jitter past ~1e63 | 🟡 Open 2026-07-06 — root cause of the user's deep-zoom nav complaints (input is provably exact, 9.5e-15px). Recentring recomputes the reference orbit → same world region redraws with slightly different iteration counts → palette amplifies to apparent drift + pan jitter. **SM-11a (DD reference/δ) TESTED and does NOT fix it** — forced across all pixels, focus-err stayed 16px byte-identical (`--navrepro scalar ddref`); precision in the 16→31-digit range is not the lever. Full model + open hypotheses + tools in **`Docs/Deep-Zoom-Perturbation.md`**. Next candidates: (a) **QD-δ variant** (mirror `ComputePixelPTRebasedDD` in QD) — the clean precision experiment; (b) **reference recycling during drag** (`AllowRefOrbitRecycle`, SM-11b) — practical UX fix for the "pan jumps" independent of the precision question; (c) tighten `--navrepro` (offset focus) to rule out the centre-on-reference measurement caveat. Gate all on `--navrepro` focus-err at 4.65e64. |
