...
| SM-11 | Deep-zoom render is reference-dependent → nav "close but not exact" / pan jitter past ~1e63 | 🟡 Deferred 2026-07-06 — root cause of the user's deep-zoom nav complaints (input is provably exact, 9.5e-15px). The rebased perturbation δ-loop `ComputePixelPTRebased` reads `Z_n` at **double** (`_refZr[m]`=X0) though the full 8-limb OD reference orbit is stored (`_refZrLo`..`_refZrX7` unused). Two nearby centres' double-rounded Z chains diverge over ~4200 iters, so recentring redraws the same region with slightly different iteration counts (rainbow palette amplifies → apparent position drift; live pan jitter). Candidate fixes, in order: (a) feed **DD `Z_n`** (Hi=`_refZr`, Lo=`_refZrLo`, already stored) into the δ recurrence `2·Zₙ·δ` + escape test — cheap, uses existing data, likely the biggest win; re-test with `--navrepro` (target: SAD(min)≈SAD(0,0)≈0, focus-err→0). (b) **reference-orbit recycling during a drag** (`AllowRefOrbitRecycle`, already implemented, default off) so all pan-preview frames share ONE reference → no jitter mid-drag. Gate everything on `--navrepro` focus-err on the user's 4.65e64 coord. NOTE: memory said DD-δ was byte-identical at 1e47 — that was an SA-seed test at shallower depth; this is a DD-**reference** test for centre-invariance at 1e64, different lever. |
