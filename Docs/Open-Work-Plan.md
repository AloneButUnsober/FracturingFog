...
| SM-5 | Extreme-tier zoom wall was 5e58 — QD is clean to 1e64 | 🟢 **Cap raised to 1e63 on 2026-07-05.** User hit a hard wall at exactly 5e58 (top-tier `QualityPreset.Extreme.ZoomMax`); wheel/outline zoom stopped. Not a bug — a stale-conservative cap ~5 decades below the proven QD floor. `--qdfloorsweep` reconfirmed 128/128 distinct through 1e64, collapse at 1e66. Raised `Extreme.ZoomMax = 1e63` (one-decade margin under 1e64 for reference-orbit accuracy in chaotic filaments). Comments in `MandelbrotCalculator` (135, 2040) corrected from ~5e58 → ~1e64. Deeper than 1e64 needs an **OD reference orbit** — see SM-6. |
| SM-6 | Zoom beyond ~1e64 needs an OD-precision reference orbit (new tier) | 🟡 Deferred 2026-07-05 — the 1e64 wall is the QD coordinate-separation floor (QD ref orbit ~62 digits). To go deeper: compute the reference orbit in **OD** (~124 digits) above ~1e60, add an "Extreme+" / "Insane" tier with `ZoomMax` ~1e116 (OD's floor per the calculator comment). Cost: OD orbit ~5–10× QD. The centre is already OD-always (ViewCamera/DeepComplex), so only the ref-orbit precision + a new preset are missing. Gate with an extended `--qdfloorsweep` giving `cxO` full 8-limb precision (current probe zeroes X4..X7, so its OD column understates OD's real reach). |
