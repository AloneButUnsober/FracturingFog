...
> Companion pages: [Technical Index](_Index.md) · [CalculatorGen Architecture](CalculatorGen-Architecture.md) · [Performance Development Plan](Performance-DevelopmentPlan.md) · [Cross-Platform Roadmap](CrossPlatform-Roadmap.md) · [Resources & Bibliography](../Resources-Bibliography.md)

---

## Precision regimes — the math behind the auto-promotion thresholds

Three numeric backends carry the calculator pipeline. The promotion thresholds between them are not
arbitrary — each one derives from how many decimal digits the underlying format can resolve and how
many digits the active zoom level demands.

### Pixel spacing as a function of zoom

The render maps a pixel grid to a complex window centred on `(C_x, C_y)`. With viewport dimensions
`W × H` and a scalar zoom `Z`, the pixel pitch is:

$$
\Delta = \frac{3.5}{\max(W,\,H)\cdot Z}
$$

The `3.5` is the historical Mandelbrot window width (`[-2.5, 1.0]` real, padded). For a 1080p view
at zoom `Z = 10^{12}`, this gives `Δ ≈ 1.7 × 10^{-15}` — right at the edge of IEEE-754 `double`.

### Decimal-digit resolution of each backend

| Format          | Mantissa bits | Decimal digits             | Pitch resolvable                  |
|-----------------|--------------:|----------------------------|-----------------------------------|
| `double`        | 53            | $\log_{10}(2^{53}) \approx 15.95$ | $\Delta \gtrsim 10^{-15}$ |
| Double-Double   | 106           | $\log_{10}(2^{106}) \approx 31.9$ | $\Delta \gtrsim 10^{-31}$ |
| Quad-Double     | 212           | $\log_{10}(2^{212}) \approx 63.8$ | $\Delta \gtrsim 10^{-63}$ |

The DD / QD types are *unevaluated sums* of IEEE-754 doubles ([Hida, Li, Bailey 2000](../Resources-Bibliography.md#deep-zoom-acceleration)). Per double you gain 53 mantissa
bits free of cost; carry costs about `~20×` per multiplication (DD) and `~75×` per multiplication
(QD) relative to scalar `double`.

### Auto-promotion thresholds

Compare each format's resolvable pitch against the live `Δ`:

$$
\boxed{\;Z_{\text{SP→DD}} \approx \frac{3.5}{W\cdot 10^{-13}} \approx 10^{12}\,\dots\,10^{13}\;}
\qquad
\boxed{\;Z_{\text{DD→QD}} \approx \frac{3.5}{W\cdot 10^{-29}} \approx 10^{25}\,\dots\,10^{27}\;}
$$

The shell uses `1e12` and `1e25` as the practical promotion gates. Below `Z_{SP→DD}` we run hot
SIMD scalar lanes (`Vector256<double>` × 4 pixels). Between `Z_{SP→DD}` and `Z_{DD→QD}` we route
to DD with the perturbation reference orbit in DD or QD (depending on iteration budget). Above
`Z_{DD→QD}` we promote to QD per-orbit-step.

> [!NOTE]
> Perturbation theory ([Martin 2014](../Resources-Bibliography.md#kevin-i-martin-superfractalthing))
> lets us push a single reference orbit through DD/QD while per-pixel deltas stay in `double` — that
> is the only reason `Z = 10^{25}` is interactive at all. See
> [CalculatorGen-Architecture.md](CalculatorGen-Architecture.md) for the symbolic δ-update derivation.

![SP → DD → QD precision regime chart. Horizontal axis is decimal zoom magnitude; vertical bands mark each backend's significant-digit envelope. The yellow strip shows where perturbation theory rescues single-precision cores at extreme zoom.](../Images/diagrams/precision-promotion.svg)

### Where the thresholds live in code

| Constant                       | Location                                                | Default     |
|--------------------------------|----------------------------------------------------------|-------------|
| `DD_PROMOTION_ZOOM`            | `Calculators/PrecisionRegime.cs`                         | `1e12`      |
| `QD_PROMOTION_ZOOM`            | `Calculators/PrecisionRegime.cs`                         | `1e25`      |
| `PERTURBATION_THRESHOLD`       | `Calculators/PerturbationDecision.cs`                    | `1e8`       |
| `SERIES_APPROX_THRESHOLD`      | `Calculators/SeriesApproximation.cs`                     | `1e15`      |

The Quality preset (Draft → Extreme) overrides these on a per-tier basis — see the *Quality
Presets* table in [User/Avalonia-UserGuide.md](../User/Avalonia-UserGuide.md#16-quality--precision).
