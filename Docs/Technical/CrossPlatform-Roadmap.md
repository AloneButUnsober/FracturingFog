...
> Companion pages: [Technical Index](_Index.md) · [Architecture Overview](Architecture-Overview.md) · [Performance Development Plan](Performance-DevelopmentPlan.md) · [Resources & Bibliography](../Resources-Bibliography.md)

> [!IMPORTANT]
> **Snapshot 2026-06-11.** Avalonia shell is the canonical UI; WinForms is deprecated and kept only
> as a fallback (`--winforms` flag). The migration is no longer the gating concern; the cross-platform
> blockers below are now the long pole. Re-verify each blocker row before resuming work — some have
> moved (PaletteBuilder.Lib is on the cusp of dropping `net10.0-windows` once `PdfPaletteExporter` is
> swapped to QuestPDF).
