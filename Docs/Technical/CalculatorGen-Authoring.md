...
> Companion pages: [Technical Index](_Index.md) · [CalculatorGen Architecture](CalculatorGen-Architecture.md) · [CalculatorGen Roadmap](CalculatorGen-Roadmap.md) · [User-facing CalcGen Guide](../User/CalcGen-UserGuide.md) · [Resources & Bibliography](../Resources-Bibliography.md)

---

## Worked end-to-end example — emitting `TricornCalculator.cs`

Walk through the full pipeline from prose math to a checked-in generated calculator. Tricorn is the
right tutorial because the math is small (one operator change) but it exercises every emitter path
(the conjugate operator changes the Jacobian sign and breaks the perturbation derivation if you do
it naively).
...
$$
\begin{aligned}
a_{n+1} &= a_n^2 - b_n^2 + c_x \\
b_{n+1} &= -2\,a_n\,b_n + c_y
\end{aligned}
$$
...
$$
\frac{\partial f}{\partial z} = 2\overline{z}, \qquad
...
$$
\delta_{n+1} = 2\overline{Z_n}\,\overline{\delta_n} + \overline{\delta_n}^{\,2} + d
$$

— note the **conjugation propagates through the δ-update**. The Differentiator pass picks this up
because it walks the AST symbolically.
...
| Flag           | Meaning                                                                         |
|----------------|---------------------------------------------------------------------------------|
| `--name`       | Class name. Generator writes `TricornCalculator : IFractalCalculator`.          |
| `--equation`   | Raw expression in CalcGen DSL. The parser auto-converts `Complex.Conjugate(z)`. |
| `--bailout`    | Squared bailout radius. `4.0` ⇒ `|z| > 2`.                                       |
| `--out`        | Target file path. Overwritten on each run.                                      |
| `--selftest`   | Emit `TricornCalculatorSelfTest.cs` (validator: scalar vs SIMD vs perturbation). |
...
```powershell
dotnet run --project CalculatorGen -- --selftest-run Calculators/Generated/TricornCalculatorSelfTest.cs
```
...
### Step 5 — register in the FractalType enum
...
case FractalType.Tricorn:
    return new TricornCalculator();
```

Rebuild. The new entry appears in the toolbar `Type` dropdown.
...
canonical image in [Wikipedia → Tricorn (mathematics)](https://en.wikipedia.org/wiki/Tricorn_(mathematics)).

| ![Hand-tuned TricornCalculator output.](../Images/examples/tricorn-handtuned.png) | ![CalcGen-emitted GeneratedTricorn output.](../Images/examples/tricorn-calcgen.png) |
|:--:|:--:|
| **Hand-tuned** `TricornCalculator` — `conj(z)² + c`. | **CalcGen-emitted** `GeneratedTricorn` — same iteration AST, scalar path. |
