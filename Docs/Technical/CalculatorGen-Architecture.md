...
> Companion pages: [Technical Index](_Index.md) · [CalculatorGen Authoring](CalculatorGen-Authoring.md) · [CalculatorGen Roadmap](CalculatorGen-Roadmap.md) · [Fractal Equation Design Guide](FractalEquation-DesignGuide.md) · [Architecture Overview](Architecture-Overview.md) · [Resources & Bibliography](../Resources-Bibliography.md)

---

## 0. Mathematics — what the emitters are actually emitting

Every generated calculator implements the canonical escape-time fixed point

$$
z_{n+1} = f(z_n, c), \qquad z_0 = 0, \qquad c = \text{pixel}
$$

…across five execution tiers (scalar / AVX2 / perturbation / BLA / ILGPU GPU). The four non-scalar
tiers are not independent rewrites; they are mechanical transforms of a single AST whose canonical
form is the scalar version.

### 0.1 Perturbation theory

Pick one *reference orbit* `Z_n` computed in high precision once per render. For every other pixel,
write its orbit as a small offset from the reference:

$$
z_n = Z_n + \delta_n
$$

Substitute into `z_{n+1} = z_n^2 + c` for the Mandelbrot case:

$$
\begin{aligned}
Z_{n+1} + \delta_{n+1} &= (Z_n + \delta_n)^2 + (C + d) \\
                       &= Z_n^2 + 2 Z_n \delta_n + \delta_n^2 + C + d
\end{aligned}
$$

Subtract `Z_{n+1} = Z_n^2 + C` from both sides:

$$
\boxed{\;\delta_{n+1} = 2 Z_n\,\delta_n + \delta_n^2 + d\;}
$$

Here `δ` and `d` are *small* (pixel-pitch sized) and computed in `double`; only `Z_n` is carried in
DD / QD. The CalculatorGen *Differentiator* pass implements this expansion symbolically for any user
equation, not just `z² + c`. For a general `f`, the perturbation update reads:

$$
\delta_{n+1} = \frac{\partial f}{\partial z}(Z_n, C)\,\delta_n + \frac{\partial f}{\partial c}(Z_n, C)\,d + \text{quadratic correction in } \delta_n
$$

The emitter substitutes the two partial-derivative AST nodes into the δ-update template and emits
both `double`-precision (per-pixel) and DD/QD (reference-orbit) lanes from the same AST.

### 0.2 Bilinear approximation (BLA)

Across iteration ranges where `Z_n` does not change drastically, the δ-update is well-approximated
by a single affine map:

$$
\delta_{n+s} \approx A_s\,\delta_n + B_s\,d
$$

The BLA prebuild walks the reference orbit and clusters consecutive iterations into the longest
prefixes whose `(A_s, B_s)` coefficients remain numerically valid. At render time a per-pixel
δ-step uses the precomputed `(A_s, B_s)` to skip `s` iterations in a single multiply-add,
collapsing thousands of inner iterations into one when applicable. See
[Heiland-Allen 2018](../Resources-Bibliography.md#deep-zoom-acceleration) for the original glitch +
rebase analysis the BLA decision uses.

### 0.3 Series approximation prelude

Before per-pixel iteration begins, expand `δ_n` as a polynomial in `d` around the reference:

$$
\delta_n(d) = a_n^{(1)}\,d + a_n^{(2)}\,d^2 + a_n^{(3)}\,d^3 + \dots
$$

Iterate the coefficients `a_n^{(k)}` in `double`. As long as the series remains accurate to
`O(\Delta)` the engine can skip to iteration `N_0` in one symbolic step. The roadmap calls for
auto-tuned series order based on observed truncation error; see
[CalculatorGen-Roadmap.md](CalculatorGen-Roadmap.md).

---

## 0.5 ILGPU JIT execution path

The GPU tier (Tier 6 in the authoring doc's table) is the newest emitter and works differently
enough from the CPU lanes that it deserves its own walkthrough.

### Compile-time

The AST is rewritten into an **ILGPU-friendly kernel template**. Constraints:

- No `Complex` — all complex algebra is split into `(zr, zi)` pairs at codegen time.
- No object allocations — everything is `struct` / `value` per `[Kernel]` semantics.
- Iteration count flows in as a kernel argument, not a constant.
- Reference orbits live in a `MemoryBuffer1D<double>` keyed by `(idx, lane)`; the perturbation
  δ-update reads from it via shared memory.

The compile-time emitter (see `CalculatorGen/Emitters/IlGpuEmitter.cs`) emits both a *scalar* device
kernel (one thread per pixel) and a *perturbation* device kernel (one thread per pixel, but reading
a shared reference orbit).

### Run-time (auto-dispatch)

```text
                         ┌────────────────────────────┐
                         │ AcceleratorProbe singleton │
                         │ tier-1: CUDA (NVIDIA)      │
                         │ tier-2: OpenCL (AMD/Intel) │
                         │ tier-3: CPU (ILGPU CPU)    │
                         │ tier-4: managed fallback   │
                         └─────────────┬──────────────┘
                                       │
              ┌────────────────────────┴────────────────────────┐
              │                                                 │
       Direct render path                              Perturbation path
       z, c packed as float2                           ref orbit in MemoryBuffer1D
       Grid = ⌈ W·H / blockSize ⌉                      Block size tuned per device
       Output = ArgbBuffer1D<int>                       Output = ArgbBuffer1D<int>
              │                                                 │
              └────────────────────────┬────────────────────────┘
                                       ▼
                          Copy DeviceToHost into BGRA
                          Standard colour post-pass on CPU
                          (devirtualized IColorMap)
```

### When the GPU tier wins

| Scenario                              | GPU advantage                                              |
|---------------------------------------|-------------------------------------------------------------|
| Iter budget ≥ 4 000, zoom ≤ `1e9`     | ~3-10× over AVX2 on a mid-range NVIDIA GPU.                |
| Large render dimensions (8K, posters) | ~5-15× — grid dispatch hides memory latency.                |
| Perturbation reference orbit reuse   | GPU wins decisively because the shared orbit fits L1.       |

### When the GPU tier loses

| Scenario                              | Why                                                        |
|---------------------------------------|-------------------------------------------------------------|
| Iter budget ≤ 500                     | Kernel launch overhead dominates; scalar AVX2 wins.        |
| DD/QD precision required (Z > 1e12)   | ILGPU device lacks DD/QD; reference orbit must stay CPU.   |
| Heavy custom equation w/ `sin`/`cos`  | GPU `sincos` precision varies by vendor; falls back to CPU. |

### Verifying GPU parity

CalculatorGen's `--selftest` flag emits a sibling validator that compares every emitter on a
fixed `64 × 64` grid. The GPU tier is checked against scalar `double` with a per-pixel max iter
delta of `1`. See `CalculatorGen/Validators/SelfTestEmitter.cs` for the harness and
`Calculators/Generated/UserDslEquationCalculatorSelfTest.cs` for an example output.
