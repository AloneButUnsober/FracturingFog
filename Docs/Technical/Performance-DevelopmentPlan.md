...
> Companion pages: [Technical Index](_Index.md) · [Architecture Overview](Architecture-Overview.md) · [CalculatorGen Architecture](CalculatorGen-Architecture.md) · [CalculatorGen Roadmap](CalculatorGen-Roadmap.md) · [Resources & Bibliography](../Resources-Bibliography.md)
...
> emitter described in [CalculatorGen-Architecture.md § 0.5](CalculatorGen-Architecture.md#05-ilgpu-jit-execution-path)
...
>
> Performance items below predate the GPU JIT. Tier-1 and Tier-2 items remain valid;
> Tier-3+ items should be re-evaluated against the GPU baseline before scheduling.
...
where $\bar{m} \le M$ is the average iteration count actually reached (escape times most pixels long
before `M`). Per-tier throughput:

| Tier             | Effective iter/sec        | Bottleneck                                  |
|------------------|---------------------------|----------------------------------------------|
| Scalar (`double`) | ~`1.0 × 10^9`             | ILP, cache pressure                          |
| AVX2 + FMA       | ~`3.5 × 10^9`             | Vector lane utilisation                      |
| AVX-512 (perturb) | ~`5.0 × 10^9`             | δ-update memory bandwidth                    |
| ILGPU CUDA (1080-class) | ~`4.0 × 10^{10}`     | Kernel launch + DeviceToHost copy            |
| ILGPU CUDA (4090-class) | ~`1.5 × 10^{11}`     | DeviceToHost copy → 8K-pitch host buffer    |
