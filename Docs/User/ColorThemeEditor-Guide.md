...
> Companion pages: [User Index](_Index.md) · [ColorGen DSL Guide](ColorGen-UserGuide.md)

![Color Theme Editor — new theme, Gradient kind selected.](../Images/dialogs/color-theme-editor.png)
...
The Theme Editor lets you edit that paint-by-numbers rule live, with the fractal repainting itself
as you drag sliders. Nothing you do here changes the math; it only changes how the math is
visualised.
...
![Seahorse Valley at zoom 10³ painted with the "MS Standard Sunrise" gradient — warm amber filaments on cool blue voids.](../Images/examples/seahorse-sunset.png)
...
3. The Image Palette helper opens, samples the photo with k-means in CIELAB colour space, and
   gives you back five evenly-spaced stops.
4. Click **Use as Stops**. The Color Stops list is replaced.
5. Tick **Live Preview** — the fractal now wears the photo's palette.

> [!TIP]
> The Image Palette helper sorts stops by hue, then by lightness. If you want a *cooler* palette
> from a *warm* photo, swap stops 1↔5 and 2↔4 by dragging their position handles.
...
5. Drag the **Key Light** direction so it falls roughly top-left. Set its **Diffuse RGB** to a
   warm white `#FFEBD0`.
...
| 3D light rig + shared Steepness / Ambient | Phong3D and Pbr3D. Light **colours** only when **3D light colour** is ticked (default on). |
...
| **3D light colour** | on | Phong3D / Pbr3D only. Include random diffuse + specular light colours. Off keeps each light's current colour while still re-placing the rig and re-rolling intensity/shininess — so you can re-roll the lighting without losing hand-picked light colours. No effect on non-3D Kinds. |
...
> [!NOTE]
> Randomize replaces the current stops and the settings it covers. Untick a scope toggle (In-set, Post-FX, Interpolation, 3D light colour) to protect those sections from being overwritten.
