...
> escape-time families, so terrain stays crisp in small windows, and it shows a
> low-res 3D preview during pan/zoom (no flat→3D flash on settle). (A
> **Compile & Load** hot-loaded equation uses the display-resolution field and
> settles to 3D without the live preview, to stay consistent with its compiled
> colour.)
