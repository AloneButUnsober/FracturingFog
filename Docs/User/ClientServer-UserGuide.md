...
> Companion pages: [User Index](_Index.md) · [Server Admin Guide](ServerAdmin-Guide.md) · [Capture Guide](Capture-Guide.md)

FFClient is a tall dialog — the connection list, certificate fields, preset chooser, render controls, and video options don't fit in one screen capture, so the guide shows it in three stitched panels.

![FFClient — top: master password, vault status, connection list, certificate path / password.](../Images/dialogs/ffclient-top.png)
*FFClient — top panel. Master password, vault status, saved connection list, certificate path + password.*

![FFClient — middle: render preset chooser through to the iteration field.](../Images/dialogs/ffclient-middle.png)
*FFClient — middle panel. Render preset, dimensions, output path, iteration field.*

![FFClient — bottom: video options through to the **Close** button.](../Images/dialogs/ffclient-bottom.png)
*FFClient — bottom panel. Video options, heartbeat indicator, **Render** / **Cancel** / **Close**.*

---

## A friendly tour

Think of Client / Server mode as **rendering at a friend's house while you watch from your couch**.

- **The Server** is a copy of Fracturing Fog running on a beefy machine (a desktop with a fast GPU,
  a workstation, a render box). It has no UI — it just sits and waits for someone to ask it to render.
- **The Client** is the dialog inside the regular UI on your laptop. You pick a server, you pick a
  region/theme/quality, and you hit **Render**. The picture is calculated on the server, sent back
  over the network, and saved to *your* disk.

Everything in between travels over **mutual TLS**. The server proves it is who it says it is; the
client proves it has been issued a valid client certificate. No certificate, no render. Saved
connection passwords on your laptop are encrypted with AES-GCM, unlocked once per session with a
master password you choose.

### Worked example — "First-ever connection"

Imagine your desktop is on `192.168.1.50` and you are on a laptop on the same LAN.

**On the desktop:**

1. Open a PowerShell prompt in the Fracturing Fog folder.
2. Run `FracturingFog.exe --server --bind 0.0.0.0`.
3. The server starts, prints its listening address, and writes a fresh certificate bundle to
   `%APPDATA%\FracturingFog\server-certs\`.
4. Copy `client.pfx` from that folder somewhere you can move it — USB stick, OneDrive, Resilio,
   anything.

**On the laptop:**

5. Move `client.pfx` somewhere private. **Do not put it in source control.**
6. Open Fracturing Fog (regular UI). Floating Menu → **Client…**
7. The dialog asks for a *master password* the first time. Pick one; you will type it once per
   session to unlock saved connections.
8. Click **Add Connection**. Pick:
   - Host: `192.168.1.50`
   - Port: the number the server printed
   - Client cert path: where you saved `client.pfx`
   - Cert password: the one the server printed (or `cert` for the first-run default)
9. Save. The connection appears in the *Connections* dropdown.

**Render a poster remotely:**

10. Pick the connection. Mode = `image`. Width × Height = `7680 × 4320`. Fractal = `Mandelbrot`.
    Region = `Seahorse Valley`. Theme = `Fire`.
11. Output path = `C:\Posters\seahorse-8k.png` *on your laptop*.
12. Click **Render**.

Progress streams from the server. When it finishes, the PNG lands on your laptop's disk. Open it.

> [!IMPORTANT]
> The first time you connect, the server's self-signed certificate is pinned. If the server's cert
> ever changes — for example because someone re-ran first-time setup — the client refuses to
> connect until you accept the new fingerprint. This is the entire point of mutual TLS; do not
> blindly trust new fingerprints.
