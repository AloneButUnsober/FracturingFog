...
public sealed class MandelboxCalculator : IFractalCalculator
{
    ...
    public void Calculate(CancellationToken ct = default)
    {
        ...
        // lifted. Area lights still fall to CPU until #492 ports the penumbra.
        ...
        // S3 (#389/#567) — physically-based thin-lens DoF. When armed, each pixel
        // averages fx.DofSamples aperture taps re-aimed through the focal point
        // (shared ThinLensDof accumulator), replacing the screen-space gather. Off
        // / aperture 0 → the single-ray path below, untouched (byte-identical).
        ...
        Parallel.For(0, height, new ParallelOptions { CancellationToken = ct }, y =>
        {
            ...
            // Trace + shade one primary ray for a thin-lens aperture tap (mirrors
            // the single-ray path below; returns the LDR colour + a linear-HDR
            // sample). Defined per-thread so hdrScratch isn't shared across rows.
                          ...
                          out float hr, out float hg, out float hb)
            {
                ...
                {
                    double dist = MandelboxDE(sx, sy, sz, scale, fixedR2, minR2, bailout2, deIter);
                ...
                double hn = eps * 2;
                double sn0 = MandelboxDE(sx + hn, sy, sz, scale, fixedR2, minR2, bailout2, deIter)
                           - MandelboxDE(sx - hn, sy, sz, scale, fixedR2, minR2, bailout2, deIter);
                double sn1 = MandelboxDE(sx, sy + hn, sz, scale, fixedR2, minR2, bailout2, deIter)
                           - MandelboxDE(sx, sy - hn, sz, scale, fixedR2, minR2, bailout2, deIter);
                double sn2 = MandelboxDE(sx, sy, sz + hn, scale, fixedR2, minR2, bailout2, deIter)
                           - MandelboxDE(sx, sy, sz - hn, scale, fixedR2, minR2, bailout2, deIter);
        ...
        // Thin-lens DoF integrates the lens itself and skips the depth/normal
        // G-buffer, so the screen-space passes keyed on it (SSAO / screen DoF /
        // edge-ink) are bypassed while it's on; tonemap/bloom still consume the
        // averaged HDR.
