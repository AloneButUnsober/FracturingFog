...
public sealed class BicomplexMandelbrotCalculator : IFractalCalculator
{
    ...
    public void Calculate(CancellationToken ct = default)
    {
        ...
        if (fx.UseGpuRender && fx.DebugAov == AovView.Beauty && !lowRes && sliceAxis == BicomplexSliceAxis.K && !fx.HasPositionalLight && !fx.HasAreaLight
        ...
        Parallel.For(0, height, new ParallelOptions { CancellationToken = ct }, y =>
        {
                          ...
                          out float hr, out float hg, out float hb)
            {
                ...
                {
                    double dist = BicomplexDE(sx, sy, sz, sliceW, sliceAxis, bailout2, deIter);
                ...
                double hn = eps * 2;
                double sn0 = BicomplexDE(sx + hn, sy, sz, sliceW, sliceAxis, bailout2, deIter)
                           - BicomplexDE(sx - hn, sy, sz, sliceW, sliceAxis, bailout2, deIter);
                double sn1 = BicomplexDE(sx, sy + hn, sz, sliceW, sliceAxis, bailout2, deIter)
                           - BicomplexDE(sx, sy - hn, sz, sliceW, sliceAxis, bailout2, deIter);
                double sn2 = BicomplexDE(sx, sy, sz + hn, sliceW, sliceAxis, bailout2, deIter)
                           - BicomplexDE(sx, sy, sz - hn, sliceW, sliceAxis, bailout2, deIter);
