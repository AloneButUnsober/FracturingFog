// UserDslEquationCalculator.cs — GENERATED by CalculatorGen
//
...
// Generated: 2026-06-20 11:10:56 UTC
//
...
//     iteration counts without colour; used by UserDslEquationCalculatorSelfTest
...
public sealed class UserDslEquationCalculator : IFractalCalculator, IDisposable
{
    ...
    public UserDslEquationCalculator(int width, int height) => Resize(width, height);
    ...
    private void IteratePixelScalar(double cr, double ci, uint[] buf, int idx)
    {
        ...
        for (; it < maxIt; it++)
        {
            ...
            double dr_new = (((zr + zr) * dr - (zi + zi) * di) + 0.0);
            double di_new = (((zr + zr) * di + (zi + zi) * dr) + 1.0);
            double zr_new = ((zr * zr - zi * zi) + (cr * 0.0 - ci * 1.0));
            double zi_new = ((zr * zi + zi * zr) + (cr * 1.0 + ci * 0.0));
            zr = zr_new; zi = zi_new;
            dr = dr_new; di = di_new;
        }
    ...
    private void IterateLaneAvx2(Vector256<double> cr, Vector256<double> ci, uint[] buf, int idx)
    {
        ...
        for (int n = 0; n < maxIt; n++)
        {
            ...
            itCounts = Avx2.Add(itCounts, Avx2.And(oneL, activeMaskL));
                ...
                Vector256<double> d_re5 = Vector256.Create(0.0);
                Vector256<double> d_re6 = Vector256.Create(1.0);
                Vector256<double> d_re7 = Avx.Add(d_re3, d_re5);
                Vector256<double> d_im8 = Avx.Add(d_im4, d_re6);
                Vector256<double> dr_new = d_re7;
                Vector256<double> di_new = d_im8;
                ...
                Vector256<double> z_re3 = Vector256.Create(0.0);
                Vector256<double> z_re4 = Vector256.Create(1.0);
                Vector256<double> z_re5 = Fma.MultiplyAddNegated(ci, z_re4, Avx.Multiply(cr, z_re3));
                Vector256<double> z_im6 = Fma.MultiplyAdd(cr, z_re4, Avx.Multiply(ci, z_re3));
                Vector256<double> z_re7 = Avx.Add(z_re1, z_re5);
                Vector256<double> z_im8 = Avx.Add(z_im2, z_im6);
        ...
        if (useExtendedRef)
        {
            ...
            do
            {
                ...
                for (n = 0; n < maxIt; n++)
                {
                    ...
                    {
                QD zr_q_new = ((zr_q * zr_q - zi_q * zi_q) + (CrQd * 0.0 - CiQd * 1.0));
                QD zi_q_new = ((zr_q * zi_q + zi_q * zr_q) + (CrQd * 1.0 + CiQd * 0.0));
        ...
        Parallel.For(0, Height, new ParallelOptions { CancellationToken = ct }, y =>
        {
            ...
            if (useSimd512Lane)
            {
                ...
                for (; x + 8 <= Width; x += 8)
                {
                    ...
                    Vector512<double> dvre5 = Vector512.Create(0.0);
                    Vector512<double> dvre6 = Vector512.Create(1.0);
                    Vector512<double> dvre7 = Avx512F.Add(dvre3, dvre5);
                    Vector512<double> dvim8 = Avx512F.Add(dvim4, dvre6);
                    Vector512<double> drv_new = dvre7;
                    Vector512<double> div_new = dvim8;
                    ...
                    Vector512<double> pre9 = Vector512.Create(0.0);
                    Vector512<double> pre10 = Vector512.Create(1.0);
                    Vector512<double> pre11 = Avx512F.FusedMultiplyAddNegated(pre10, ei_v, Avx512F.Multiply(pre9, er_v));
                    Vector512<double> pim12 = Avx512F.FusedMultiplyAdd(pre9, ei_v, Avx512F.Multiply(pre10, er_v));
                    Vector512<double> pre13 = Avx512F.Add(pre7, pre11);
                    Vector512<double> pim14 = Avx512F.Add(pim8, pim12);
                    Vector512<double> dr_new = pre13;
                    Vector512<double> di_new = pim14;
                            Vector512<double> keep = activeMask.AsDouble();
            ...
            else if (useSimd2Lane)
            {
                ...
                for (; x + 4 <= Width; x += 4)
                {
                    ...
                    Vector256<double> dv2im4 = Fma.MultiplyAdd(dv2re1, div, Avx.Multiply(dv2im2, drv));
                    Vector256<double> dv2re5 = Vector256.Create(0.0);
                    Vector256<double> dv2re6 = Vector256.Create(1.0);
                    Vector256<double> dv2re7 = Avx.Add(dv2re3, dv2re5);
                    Vector256<double> dv2im8 = Avx.Add(dv2im4, dv2re6);
                    Vector256<double> drv_new = dv2re7;
                    Vector256<double> div_new = dv2im8;
                    ...
                    Vector256<double> p2re3 = Fma.MultiplyAddNegated(p2im2, di, Avx.Multiply(p2re1, dr));
                    Vector256<double> p2im4 = Fma.MultiplyAdd(p2re1, di, Avx.Multiply(p2im2, dr));
                    Vector256<double> p2re5 = Fma.MultiplyAddNegated(di, di, Avx.Multiply(dr, dr));
                    Vector256<double> p2im6 = Fma.MultiplyAdd(dr, di, Avx.Multiply(di, dr));
                    Vector256<double> p2re7 = Avx.Add(p2re3, p2re5);
                    Vector256<double> p2im8 = Avx.Add(p2im4, p2im6);
                    Vector256<double> p2re9 = Vector256.Create(0.0);
                    Vector256<double> p2re10 = Vector256.Create(1.0);
                    Vector256<double> p2re11 = Fma.MultiplyAddNegated(p2re10, ei_v, Avx.Multiply(p2re9, er_v));
                    Vector256<double> p2im12 = Fma.MultiplyAdd(p2re9, ei_v, Avx.Multiply(p2re10, er_v));
                    Vector256<double> p2re13 = Avx.Add(p2re7, p2re11);
                    Vector256<double> p2im14 = Avx.Add(p2im8, p2im12);
                    Vector256<double> dr_new = p2re13;
                    Vector256<double> di_new = p2im14;
                            Vector256<double> keep = activeMask.AsDouble();
            ...
            for (; x < Width; x++)
            {
                ...
                double dr = 0.0, di = 0.0;     // δ
                {
                    ...
                    for (; it < refOrbitLen; it++)
                    {
                        ...
                        {
                    double drv_new = (((zr + zr) * drv - (zi + zi) * div) + 0.0);
                    double div_new = (((zr + zr) * div + (zi + zi) * drv) + 1.0);
                    ...
                    double dr_new = ((((Zr + Zr) * dr - (Zi + Zi) * di) + (dr * dr - di * di)) + (0.0 * er - 1.0 * ei));
                    double di_new = ((((Zr + Zr) * di + (Zi + Zi) * dr) + (dr * di + di * dr)) + (0.0 * ei + 1.0 * er));
                        dr = dr_new; di = di_new;
                    }
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    DD zr_dd_new = ((zr_dd * zr_dd - zi_dd * zi_dd) + (cr_dd * 0.0 - ci_dd * 1.0));
                    DD zi_dd_new = ((zr_dd * zi_dd + zi_dd * zr_dd) + (cr_dd * 1.0 + ci_dd * 0.0));
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    QD zr_q_new = ((zr_q * zr_q - zi_q * zi_q) + (cr_q * 0.0 - ci_q * 1.0));
                    QD zi_q_new = ((zr_q * zi_q + zi_q * zr_q) + (cr_q * 1.0 + ci_q * 0.0));
            zr_q = zr_q_new; zi_q = zi_q_new;
        }
        return ColorForDd(it, finalZr, finalZi, finalZMag2, drv, div, maxIt, bufIdx);
    }
