// MandelbrotBurningShipCalculator.cs — GENERATED by CalculatorGen
//
// Source equation (z, c complex):
//     z_{n+1} = if abs(z) > 4 then z else z*z + c
//
// Symbolic derivatives (auto-derived):
//     ∂p/∂z = if abs(z) > 4 then 1 else z + z
//     ∂p/∂c = if abs(z) > 4 then 0 else 1
//
...
// Generated: 2026-06-20 11:10:51 UTC
//
...
//     iteration counts without colour; used by MandelbrotBurningShipCalculatorSelfTest
...
public sealed class MandelbrotBurningShipCalculator : IFractalCalculator, IDisposable
{
    ...
    private const bool SupportsDd4Direct = false;
    ...
    public MandelbrotBurningShipCalculator(int width, int height) => Resize(width, height);
    ...
    private void IteratePixelScalar(double cr, double ci, uint[] buf, int idx)
    {
        ...
        for (; it < maxIt; it++)
        {
            ...
            double dr_new = ((((((zr * zr) + zi * zi) > 4.0) ? 1.0 : (zr + zr)) * dr - ((((zr * zr) + zi * zi) > 4.0) ? 0.0 : (zi + zi)) * di) + ((((zr * zr) + zi * zi) > 4.0) ? 0.0 : 1.0));
            double di_new = (((((zr * zr) + zi * zi) > 4.0) ? 1.0 : (zr + zr)) * di + ((((zr * zr) + zi * zi) > 4.0) ? 0.0 : (zi + zi)) * dr);
            double zr_new = ((((zr * zr) + zi * zi) > 4.0) ? zr : ((zr * zr - zi * zi) + cr));
            double zi_new = ((((zr * zr) + zi * zi) > 4.0) ? zi : ((zr * zi + zi * zr) + ci));
            zr = zr_new; zi = zi_new;
            dr = dr_new; di = di_new;
        }
    ...
    private void IterateLaneAvx2(Vector256<double> cr, Vector256<double> ci, uint[] buf, int idx)
    {
        ...
        for (int n = 0; n < maxIt; n++)
        {
            ...
            itCounts = Avx2.Add(itCounts, Avx2.And(oneL, activeMaskL));
                ...
                Vector256<double> d_re2 = Avx.Add(zr, zr);
                Vector256<double> d_im3 = Avx.Add(zi, zi);
                Vector256<double> d_re4 = Fma.MultiplyAdd(zr, zr, Avx.Multiply(zi, zi));
                Vector256<double> d_re5 = Vector256.Create(4.0);
                Vector256<double> d_re6 = Avx.Compare(d_re4, d_re5, FloatComparisonMode.OrderedGreaterThanNonSignaling);
                Vector256<double> d_re7 = Vector256.ConditionalSelect(d_re6, d_re1, d_re2);
                Vector256<double> d_im8 = Vector256.ConditionalSelect(d_re6, Vector256<double>.Zero, d_im3);
                ...
                Vector256<double> d_re11 = Vector256.Create(0.0);
                Vector256<double> d_re12 = Vector256.Create(1.0);
                Vector256<double> d_re13 = Fma.MultiplyAdd(zr, zr, Avx.Multiply(zi, zi));
                Vector256<double> d_re14 = Vector256.Create(4.0);
                Vector256<double> d_re15 = Avx.Compare(d_re13, d_re14, FloatComparisonMode.OrderedGreaterThanNonSignaling);
                Vector256<double> d_re16 = Vector256.ConditionalSelect(d_re15, d_re11, d_re12);
                Vector256<double> d_im17 = Vector256.ConditionalSelect(d_re15, Vector256<double>.Zero, Vector256<double>.Zero);
                Vector256<double> d_re18 = Avx.Add(d_re9, d_re16);
                Vector256<double> d_im19 = Avx.Add(d_im10, d_im17);
                Vector256<double> dr_new = d_re18;
                Vector256<double> di_new = d_im19;
                ...
                Vector256<double> z_re5 = Fma.MultiplyAdd(zr, zr, Avx.Multiply(zi, zi));
                Vector256<double> z_re6 = Vector256.Create(4.0);
                Vector256<double> z_re7 = Avx.Compare(z_re5, z_re6, FloatComparisonMode.OrderedGreaterThanNonSignaling);
                Vector256<double> z_re8 = Vector256.ConditionalSelect(z_re7, zr, z_re3);
                Vector256<double> z_im9 = Vector256.ConditionalSelect(z_re7, zi, z_im4);
                Vector256<double> zr_new = z_re8;
                Vector256<double> zi_new = z_im9;
            ...
            do
            {
                ...
                for (n = 0; n < maxIt; n++)
                {
                    ...
                    if (UseBla)
                    {
                        ...
                        double cr = Cr, ci = Ci;
                    double Ar_new = ((((zr * zr) + zi * zi) > 4.0) ? 1.0 : (zr + zr));
                    double Ai_new = ((((zr * zr) + zi * zi) > 4.0) ? 0.0 : (zi + zi));
                    double Br_new = ((((zr * zr) + zi * zi) > 4.0) ? 0.0 : 1.0);
                    double Bi_new = 0.0;
                        double rN = BlaRelative * Math.Sqrt(r2);
                        blaLevel0![n] = new Bla(Ar_new, Ai_new, Br_new, Bi_new, rN * rN, 1);
                    }
                    {
                QD zr_q_new = (((((zr_q).X0 * (zr_q).X0) + (zi_q).X0 * (zi_q).X0) > 4.0) ? (zr_q) : (((zr_q * zr_q - zi_q * zi_q) + CrQd)));
                QD zi_q_new = (((((zr_q).X0 * (zr_q).X0) + (zi_q).X0 * (zi_q).X0) > 4.0) ? (zi_q) : (((zr_q * zi_q + zi_q * zr_q) + CiQd)));
...
#pragma warning restore CS0219
        ...
        else if (SaEnabled && UseSa && refOrbitLen > 8)
        {
            ...
            for (n = 0; n < refOrbitLen; n++)
            {
                ...
                double SrNew9 = 0.0, SiNew9 = 0.0;
                double SrNew10 = 0.0, SiNew10 = 0.0;
                double SrNew11 = 0.0, SiNew11 = 0.0;
                double SrNew12 = 0.0, SiNew12 = 0.0;
                double SrNew13 = 0.0, SiNew13 = 0.0;
                double SrNew14 = 0.0, SiNew14 = 0.0;
                double SrNew15 = 0.0, SiNew15 = 0.0;
                double SrNew16 = 0.0, SiNew16 = 0.0;
        ...
        Parallel.For(0, Height, new ParallelOptions { CancellationToken = ct }, y =>
        {
            ...
            if (useSimd512Lane)
            {
                ...
                for (; x + 8 <= Width; x += 8)
                {
                    ...
                    Vector512<double> dvre1 = Vector512.Create(1.0);
                    Vector512<double> dvre2 = Avx512F.Add(zr_v, zr_v);
                    Vector512<double> dvim3 = Avx512F.Add(zi_v, zi_v);
                    Vector512<double> dvre4 = Avx512F.FusedMultiplyAdd(zr_v, zr_v, Avx512F.Multiply(zi_v, zi_v));
                    Vector512<double> dvre5 = Vector512.Create(4.0);
                    Vector512<double> dvre6 = Avx512F.Compare(dvre4, dvre5, FloatComparisonMode.OrderedGreaterThanNonSignaling);
                    Vector512<double> dvre7 = Vector512.ConditionalSelect(dvre6, dvre1, dvre2);
                    Vector512<double> dvim8 = Vector512.ConditionalSelect(dvre6, Vector512<double>.Zero, dvim3);
                    ...
                    Vector512<double> dvre11 = Vector512.Create(0.0);
                    Vector512<double> dvre12 = Vector512.Create(1.0);
                    Vector512<double> dvre13 = Avx512F.FusedMultiplyAdd(zr_v, zr_v, Avx512F.Multiply(zi_v, zi_v));
                    Vector512<double> dvre14 = Vector512.Create(4.0);
                    Vector512<double> dvre15 = Avx512F.Compare(dvre13, dvre14, FloatComparisonMode.OrderedGreaterThanNonSignaling);
                    Vector512<double> dvre16 = Vector512.ConditionalSelect(dvre15, dvre11, dvre12);
                    Vector512<double> dvim17 = Vector512.ConditionalSelect(dvre15, Vector512<double>.Zero, Vector512<double>.Zero);
                    Vector512<double> dvre18 = Avx512F.Add(dvre9, dvre16);
                    Vector512<double> dvim19 = Avx512F.Add(dvim10, dvim17);
                    Vector512<double> drv_new = dvre18;
                    Vector512<double> div_new = dvim19;
            ...
            else if (useSimd2Lane)
            {
                ...
                for (; x + 4 <= Width; x += 4)
                {
                    ...
                    Vector256<double> dv2re1 = Vector256.Create(1.0);
                    Vector256<double> dv2re2 = Avx.Add(zr_v, zr_v);
                    Vector256<double> dv2im3 = Avx.Add(zi_v, zi_v);
                    Vector256<double> dv2re4 = Fma.MultiplyAdd(zr_v, zr_v, Avx.Multiply(zi_v, zi_v));
                    Vector256<double> dv2re5 = Vector256.Create(4.0);
                    Vector256<double> dv2re6 = Avx.Compare(dv2re4, dv2re5, FloatComparisonMode.OrderedGreaterThanNonSignaling);
                    Vector256<double> dv2re7 = Vector256.ConditionalSelect(dv2re6, dv2re1, dv2re2);
                    Vector256<double> dv2im8 = Vector256.ConditionalSelect(dv2re6, Vector256<double>.Zero, dv2im3);
                    ...
                    Vector256<double> dv2re11 = Vector256.Create(0.0);
                    Vector256<double> dv2re12 = Vector256.Create(1.0);
                    Vector256<double> dv2re13 = Fma.MultiplyAdd(zr_v, zr_v, Avx.Multiply(zi_v, zi_v));
                    Vector256<double> dv2re14 = Vector256.Create(4.0);
                    Vector256<double> dv2re15 = Avx.Compare(dv2re13, dv2re14, FloatComparisonMode.OrderedGreaterThanNonSignaling);
                    Vector256<double> dv2re16 = Vector256.ConditionalSelect(dv2re15, dv2re11, dv2re12);
                    Vector256<double> dv2im17 = Vector256.ConditionalSelect(dv2re15, Vector256<double>.Zero, Vector256<double>.Zero);
                    Vector256<double> dv2re18 = Avx.Add(dv2re9, dv2re16);
                    Vector256<double> dv2im19 = Avx.Add(dv2im10, dv2im17);
                    Vector256<double> drv_new = dv2re18;
                    Vector256<double> div_new = dv2im19;
            ...
            for (; x < Width; x++)
            {
                ...
                double dr = 0.0, di = 0.0;     // δ
                {
                    ...
                    for (; it < refOrbitLen; it++)
                    {
                        ...
                        {
                    double drv_new = ((((((zr * zr) + zi * zi) > 4.0) ? (1.0) : ((zr + zr))) * drv - ((((zr * zr) + zi * zi) > 4.0) ? 0.0 : ((zi + zi))) * div) + ((((zr * zr) + zi * zi) > 4.0) ? (0.0) : (1.0)));
                    double div_new = (((((zr * zr) + zi * zi) > 4.0) ? (1.0) : ((zr + zr))) * div + ((((zr * zr) + zi * zi) > 4.0) ? 0.0 : ((zi + zi))) * drv);
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    DD zr_dd_new = (((((zr_dd).Hi * (zr_dd).Hi) + (zi_dd).Hi * (zi_dd).Hi) > 4.0) ? (zr_dd) : (((zr_dd * zr_dd - zi_dd * zi_dd) + cr_dd)));
                    DD zi_dd_new = (((((zr_dd).Hi * (zr_dd).Hi) + (zi_dd).Hi * (zi_dd).Hi) > 4.0) ? (zi_dd) : (((zr_dd * zi_dd + zi_dd * zr_dd) + ci_dd)));
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    QD zr_q_new = (((((zr_q).X0 * (zr_q).X0) + (zi_q).X0 * (zi_q).X0) > 4.0) ? (zr_q) : (((zr_q * zr_q - zi_q * zi_q) + cr_q)));
                    QD zi_q_new = (((((zr_q).X0 * (zr_q).X0) + (zi_q).X0 * (zi_q).X0) > 4.0) ? (zi_q) : (((zr_q * zi_q + zi_q * zr_q) + ci_q)));
            zr_q = zr_q_new; zi_q = zi_q_new;
        }
        return ColorForDd(it, finalZr, finalZi, finalZMag2, drv, div, maxIt, bufIdx);
    }
