// MandelbrotPhoenixCalculator.cs — GENERATED by CalculatorGen
//
// Source equation (z, c complex):
//     z_{n+1} = z*z + c + 0.5*prev
//
...
// Generated: 2026-06-20 11:10:48 UTC
//
...
//     iteration counts without colour; used by MandelbrotPhoenixCalculatorSelfTest
...
public sealed class MandelbrotPhoenixCalculator : IFractalCalculator, IDisposable
{
    ...
    private void IteratePixelScalar(double cr, double ci, uint[] buf, int idx)
    {
        ...
        for (; it < maxIt; it++)
        {
            ...
            double zr_new = (((zr * zr - zi * zi) + cr) + (0.5 * pr));
            double zi_new = (((zr * zi + zi * zr) + ci) + (0.5 * pi));
            pr = zr; pi = zi;
            zr = zr_new; zi = zi_new;
            dr = dr_new; di = di_new;
        }
    ...
    private void IterateLaneAvx2(Vector256<double> cr, Vector256<double> ci, uint[] buf, int idx)
    {
        ...
        for (int n = 0; n < maxIt; n++)
        {
            ...
            itCounts = Avx2.Add(itCounts, Avx2.And(oneL, activeMaskL));
                ...
                Vector256<double> z_re5 = Vector256.Create(0.5);
                Vector256<double> z_re6 = Avx.Multiply(z_re5, pr);
                Vector256<double> z_im7 = Avx.Multiply(z_re5, pi);
                Vector256<double> z_re8 = Avx.Add(z_re3, z_re6);
                Vector256<double> z_im9 = Avx.Add(z_im4, z_im7);
        ...
        if (useExtendedRef)
        {
            ...
            QD pr_q = QD.Zero, pi_q = QD.Zero;
                ...
                for (n = 0; n < maxIt; n++)
                {
                    ...
                    {
                QD zr_q_new = (((zr_q * zr_q - zi_q * zi_q) + CrQd) + (0.5 * pr_q));
                QD zi_q_new = (((zr_q * zi_q + zi_q * zr_q) + CiQd) + (0.5 * pi_q));
                    pr_q = zr_q; pi_q = zi_q;
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    DD zr_dd_new = (((zr_dd * zr_dd - zi_dd * zi_dd) + cr_dd) + (0.5 * pr_dd));
                    DD zi_dd_new = (((zr_dd * zi_dd + zi_dd * zr_dd) + ci_dd) + (0.5 * pi_dd));
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    QD zr_q_new = (((zr_q * zr_q - zi_q * zi_q) + cr_q) + (0.5 * pr_q));
                    QD zi_q_new = (((zr_q * zi_q + zi_q * zr_q) + ci_q) + (0.5 * pi_q));
            pr_q = zr_q; pi_q = zi_q;
            zr_q = zr_q_new; zi_q = zi_q_new;
        }
        return ColorForDd(it, finalZr, finalZi, finalZMag2, drv, div, maxIt, bufIdx);
