// BurningShipCalculator.cs — GENERATED by CalculatorGen
//
// Source equation (z, c complex):
//     z_{n+1} = fold(z)*fold(z) + c
//
...
// Generated: 2026-06-20 11:10:52 UTC
//
...
//     iteration counts without colour; used by BurningShipCalculatorSelfTest
...
public sealed class BurningShipCalculator : IFractalCalculator, IDisposable
{
    ...
    public BurningShipCalculator(int width, int height) => Resize(width, height);
    ...
    private void IteratePixelScalar(double cr, double ci, uint[] buf, int idx)
    {
        ...
        for (; it < maxIt; it++)
        {
            ...
            double dr_new = 1.0;
            double di_new = 0.0;
            double zr_new = ((Math.Abs(zr) * Math.Abs(zr) - Math.Abs(zi) * Math.Abs(zi)) + cr);
            double zi_new = ((Math.Abs(zr) * Math.Abs(zi) + Math.Abs(zi) * Math.Abs(zr)) + ci);
            zr = zr_new; zi = zi_new;
            dr = dr_new; di = di_new;
        }
    ...
    private void IterateLaneAvx2(Vector256<double> cr, Vector256<double> ci, uint[] buf, int idx)
    {
        ...
        for (int n = 0; n < maxIt; n++)
        {
            ...
            itCounts = Avx2.Add(itCounts, Avx2.And(oneL, activeMaskL));
                ...
                Vector256<double> z_re1 = Avx.AndNot(Vector256.Create(-0.0), zr);
                Vector256<double> z_im2 = Avx.AndNot(Vector256.Create(-0.0), zi);
                Vector256<double> z_re3 = Avx.AndNot(Vector256.Create(-0.0), zr);
                Vector256<double> z_im4 = Avx.AndNot(Vector256.Create(-0.0), zi);
                Vector256<double> z_re5 = Fma.MultiplyAddNegated(z_im2, z_im4, Avx.Multiply(z_re1, z_re3));
                Vector256<double> z_im6 = Fma.MultiplyAdd(z_re1, z_im4, Avx.Multiply(z_im2, z_re3));
        ...
        if (useExtendedRef)
        {
            ...
            do
            {
                ...
                for (n = 0; n < maxIt; n++)
                {
                    ...
                    {
                QD zr_q_new = (((zr_q.X0 < 0.0 ? -(zr_q) : (zr_q)) * (zr_q.X0 < 0.0 ? -(zr_q) : (zr_q)) - (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q)) * (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q))) + CrQd);
                QD zi_q_new = (((zr_q.X0 < 0.0 ? -(zr_q) : (zr_q)) * (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q)) + (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q)) * (zr_q.X0 < 0.0 ? -(zr_q) : (zr_q))) + CiQd);
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    DD zr_dd_new = (((zr_dd.Hi < 0.0 ? -(zr_dd) : (zr_dd)) * (zr_dd.Hi < 0.0 ? -(zr_dd) : (zr_dd)) - (zi_dd.Hi < 0.0 ? -(zi_dd) : (zi_dd)) * (zi_dd.Hi < 0.0 ? -(zi_dd) : (zi_dd))) + cr_dd);
                    DD zi_dd_new = (((zr_dd.Hi < 0.0 ? -(zr_dd) : (zr_dd)) * (zi_dd.Hi < 0.0 ? -(zi_dd) : (zi_dd)) + (zi_dd.Hi < 0.0 ? -(zi_dd) : (zi_dd)) * (zr_dd.Hi < 0.0 ? -(zr_dd) : (zr_dd))) + ci_dd);
        ...
        for (; it < maxIt; it++)
        {
            ...
            }
            {
                ...
                drv = drv_new; div = div_new;
                    ...
                    QD zr_q_new = (((zr_q.X0 < 0.0 ? -(zr_q) : (zr_q)) * (zr_q.X0 < 0.0 ? -(zr_q) : (zr_q)) - (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q)) * (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q))) + cr_q);
                    QD zi_q_new = (((zr_q.X0 < 0.0 ? -(zr_q) : (zr_q)) * (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q)) + (zi_q.X0 < 0.0 ? -(zi_q) : (zi_q)) * (zr_q.X0 < 0.0 ? -(zr_q) : (zr_q))) + ci_q);
            zr_q = zr_q_new; zi_q = zi_q_new;
        }
        return ColorForDd(it, finalZr, finalZi, finalZMag2, drv, div, maxIt, bufIdx);
    }
