...
{
    private Action<Index1D, ArrayView<uint>, GpuRaymarchParams, GpuShadingParams, QMandelGpuParams, ArrayView<uint>>? _kernel;
    ...
    private bool TryInit()
    {
        ...
        try
        {
            _kernel = acc.LoadAutoGroupedStreamKernel<
                Index1D, ArrayView<uint>, GpuRaymarchParams, GpuShadingParams, QMandelGpuParams, ArrayView<uint>>(QMandelKernel);
            return true;
        }
        catch (Exception ex)
        {
            LastError = $"QMandel GPU kernel load failed: {ex.GetBaseException().Message}";
            _initFailed = true;
            return false;
        }
    }

    public bool Render(uint[] outBuffer, GpuRaymarchParams r, GpuShadingParams sp, QMandelGpuParams p, uint[]? palette = null)
    {
    ...
    private static void QMandelKernel(
        Index1D idx, ArrayView<uint> output, GpuRaymarchParams r, GpuShadingParams sp, QMandelGpuParams p, ArrayView<uint> palette)
    {
        int x = idx % r.Width;
        int y = idx / r.Width;
