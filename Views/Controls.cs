...
namespace FracturingFog.Views
{
    ...
    public static class FormHelpers
    {
        ...
        //
        // Implementation lives in FracturingFog.Abstractions.Math.QdCoordCodec
        // (cross-platform, pure BCL) so both this WinForms host and the
        // Avalonia cross-platform host (FracturingFog.Hosting.AvaloniaDialogs)
        // share one impl. These thin shims preserve the legacy
        // FracturingFog.Views.FormHelpers API for in-tree callers.

        public static string FormatCoordSingle(double hi, double lo, double x2, double x3)
            => global::FracturingFog.Abstractions.Math.QdCoordCodec.FormatCoordSingle(hi, lo, x2, x3);

        public static bool TryParseCoordSingle(string text,
            out double hi, out double lo, out double x2, out double x3)
            => global::FracturingFog.Abstractions.Math.QdCoordCodec.TryParseCoordSingle(text, out hi, out lo, out x2, out x3);

        public static bool TryParseCoordAny(string text,
            out double hi, out double lo, out double x2, out double x3)
            => global::FracturingFog.Abstractions.Math.QdCoordCodec.TryParseCoordAny(text, out hi, out lo, out x2, out x3);
