...
public sealed class MandelbrotZ5Calculator : IFractalCalculator, IDisposable
{
    ...
    private void IteratePixelScalar(double cr, double ci, uint[] buf, int idx)
    {
        ...
        for (; it < maxIt; it++)
        {
            ...
            double dr_new = (((5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zr - ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zi)) * dr - (5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zi + ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zr)) * di) + 1.0);
            double di_new = ((5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zr - ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zi)) * di + (5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zi + ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zr)) * dr);
    ...
    private void IterateLaneAvx2(Vector256<double> cr, Vector256<double> ci, uint[] buf, int idx)
    {
        ...
        for (int n = 0; n < maxIt; n++)
        {
            ...
            itCounts = Avx2.Add(itCounts, Avx2.And(oneL, activeMaskL));
                ...
                Vector256<double> d_re1 = Vector256.Create(5.0);
                Vector256<double> d_re2 = Fma.MultiplyAddNegated(zi, zi, Avx.Multiply(zr, zr));
                Vector256<double> d_im3 = Fma.MultiplyAdd(zr, zi, Avx.Multiply(zi, zr));
                Vector256<double> d_re4 = Fma.MultiplyAddNegated(d_im3, zi, Avx.Multiply(d_re2, zr));
                Vector256<double> d_im5 = Fma.MultiplyAdd(d_re2, zi, Avx.Multiply(d_im3, zr));
                Vector256<double> d_re6 = Fma.MultiplyAddNegated(d_im5, zi, Avx.Multiply(d_re4, zr));
                Vector256<double> d_im7 = Fma.MultiplyAdd(d_re4, zi, Avx.Multiply(d_im5, zr));
                Vector256<double> d_re8 = Avx.Multiply(d_re1, d_re6);
                Vector256<double> d_im9 = Avx.Multiply(d_re1, d_im7);
                Vector256<double> d_re10 = Fma.MultiplyAddNegated(d_im9, di, Avx.Multiply(d_re8, dr));
                Vector256<double> d_im11 = Fma.MultiplyAdd(d_re8, di, Avx.Multiply(d_im9, dr));
                Vector256<double> d_re12 = Vector256.Create(1.0);
                Vector256<double> d_re13 = Avx.Add(d_re10, d_re12);
                Vector256<double> dr_new = d_re13;
                Vector256<double> di_new = d_im11;
    ...
    //                 =  ε + 5*z^4*δ + 0.5*5*4*z^3*δ*δ + 0.16666666666666666*5*4*3*z^2*δ*δ*δ + 0.041666666666666664*5*4*3*2*z*δ*δ*δ*δ + δ*δ*δ*δ*δ
    //
        ...
        if (useExtendedRef)
        {
            ...
            for (n = 0; n < maxIt; n++)
            {
                ...
                if (UseBla)
                {
                    ...
                    double cr = Cr, ci = Ci;
                    double Ar_new = (5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zr - ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zi));
                    double Ai_new = (5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zi + ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zr));
                    double Br_new = 1.0;
                    double Bi_new = 0.0;
            ...
            if (useSimdLane)
            {
                ...
                for (; x + 8 <= Width; x += 8)
                {
                    ...
                    for (int it = 0; it < refOrbitLen; it++)
                    {
                        ...
                        // Derivative
                        {
                    Vector512<double> dvre1 = Vector512.Create(5.0);
                    Vector512<double> dvre2 = Avx512F.FusedMultiplyAddNegated(zi_v, zi_v, Avx512F.Multiply(zr_v, zr_v));
                    Vector512<double> dvim3 = Avx512F.FusedMultiplyAdd(zr_v, zi_v, Avx512F.Multiply(zi_v, zr_v));
                    Vector512<double> dvre4 = Avx512F.FusedMultiplyAddNegated(dvim3, zi_v, Avx512F.Multiply(dvre2, zr_v));
                    Vector512<double> dvim5 = Avx512F.FusedMultiplyAdd(dvre2, zi_v, Avx512F.Multiply(dvim3, zr_v));
                    Vector512<double> dvre6 = Avx512F.FusedMultiplyAddNegated(dvim5, zi_v, Avx512F.Multiply(dvre4, zr_v));
                    Vector512<double> dvim7 = Avx512F.FusedMultiplyAdd(dvre4, zi_v, Avx512F.Multiply(dvim5, zr_v));
                    Vector512<double> dvre8 = Avx512F.Multiply(dvre1, dvre6);
                    Vector512<double> dvim9 = Avx512F.Multiply(dvre1, dvim7);
                    Vector512<double> dvre10 = Avx512F.FusedMultiplyAddNegated(dvim9, div, Avx512F.Multiply(dvre8, drv));
                    Vector512<double> dvim11 = Avx512F.FusedMultiplyAdd(dvre8, div, Avx512F.Multiply(dvim9, drv));
                    Vector512<double> dvre12 = Vector512.Create(1.0);
                    Vector512<double> dvre13 = Avx512F.Add(dvre10, dvre12);
                    Vector512<double> drv_new = dvre13;
                    Vector512<double> div_new = dvim11;
                        ...
                        // δ
                        {
                    Vector512<double> pre1 = Vector512.Create(5.0);
                    Vector512<double> pre2 = Avx512F.FusedMultiplyAddNegated(Zi_v, Zi_v, Avx512F.Multiply(Zr_v, Zr_v));
                    Vector512<double> pim3 = Avx512F.FusedMultiplyAdd(Zr_v, Zi_v, Avx512F.Multiply(Zi_v, Zr_v));
                    Vector512<double> pre4 = Avx512F.FusedMultiplyAddNegated(pim3, Zi_v, Avx512F.Multiply(pre2, Zr_v));
                    Vector512<double> pim5 = Avx512F.FusedMultiplyAdd(pre2, Zi_v, Avx512F.Multiply(pim3, Zr_v));
                    Vector512<double> pre6 = Avx512F.FusedMultiplyAddNegated(pim5, Zi_v, Avx512F.Multiply(pre4, Zr_v));
                    Vector512<double> pim7 = Avx512F.FusedMultiplyAdd(pre4, Zi_v, Avx512F.Multiply(pim5, Zr_v));
                    Vector512<double> pre8 = Avx512F.Multiply(pre1, pre6);
                    Vector512<double> pim9 = Avx512F.Multiply(pre1, pim7);
                    Vector512<double> pre10 = Avx512F.FusedMultiplyAddNegated(pim9, di, Avx512F.Multiply(pre8, dr));
                    Vector512<double> pim11 = Avx512F.FusedMultiplyAdd(pre8, di, Avx512F.Multiply(pim9, dr));
                    Vector512<double> pre12 = Avx512F.Add(er_v, pre10);
                    Vector512<double> pim13 = Avx512F.Add(ei_v, pim11);
                    Vector512<double> pre14 = Vector512.Create(0.5);
                    Vector512<double> pre15 = Vector512.Create(5.0);
                    Vector512<double> pre16 = Vector512.Create(4.0);
                    Vector512<double> pre17 = Avx512F.FusedMultiplyAddNegated(Zi_v, Zi_v, Avx512F.Multiply(Zr_v, Zr_v));
                    Vector512<double> pim18 = Avx512F.FusedMultiplyAdd(Zr_v, Zi_v, Avx512F.Multiply(Zi_v, Zr_v));
                    Vector512<double> pre19 = Avx512F.FusedMultiplyAddNegated(pim18, Zi_v, Avx512F.Multiply(pre17, Zr_v));
                    Vector512<double> pim20 = Avx512F.FusedMultiplyAdd(pre17, Zi_v, Avx512F.Multiply(pim18, Zr_v));
                    Vector512<double> pre21 = Avx512F.Multiply(pre16, pre19);
                    Vector512<double> pim22 = Avx512F.Multiply(pre16, pim20);
                    Vector512<double> pre23 = Avx512F.Multiply(pre15, pre21);
                    Vector512<double> pim24 = Avx512F.Multiply(pre15, pim22);
                    Vector512<double> pre25 = Avx512F.Multiply(pre14, pre23);
                    Vector512<double> pim26 = Avx512F.Multiply(pre14, pim24);
                    ...
                    Vector512<double> pre31 = Avx512F.Add(pre12, pre29);
                    Vector512<double> pim32 = Avx512F.Add(pim13, pim30);
                    Vector512<double> pre33 = Vector512.Create(0.16666666666666666);
                    Vector512<double> pre34 = Vector512.Create(5.0);
                    Vector512<double> pre35 = Vector512.Create(4.0);
                    Vector512<double> pre36 = Vector512.Create(3.0);
                    ...
                    Vector512<double> pre39 = Avx512F.Multiply(pre36, pre37);
                    Vector512<double> pim40 = Avx512F.Multiply(pre36, pim38);
                    Vector512<double> pre41 = Avx512F.Multiply(pre35, pre39);
                    Vector512<double> pim42 = Avx512F.Multiply(pre35, pim40);
                    Vector512<double> pre43 = Avx512F.Multiply(pre34, pre41);
                    Vector512<double> pim44 = Avx512F.Multiply(pre34, pim42);
                    Vector512<double> pre45 = Avx512F.Multiply(pre33, pre43);
                    Vector512<double> pim46 = Avx512F.Multiply(pre33, pim44);
                    Vector512<double> pre47 = Avx512F.FusedMultiplyAddNegated(pim46, di, Avx512F.Multiply(pre45, dr));
                    Vector512<double> pim48 = Avx512F.FusedMultiplyAdd(pre45, di, Avx512F.Multiply(pim46, dr));
                    Vector512<double> pre49 = Avx512F.FusedMultiplyAddNegated(pim48, di, Avx512F.Multiply(pre47, dr));
                    Vector512<double> pim50 = Avx512F.FusedMultiplyAdd(pre47, di, Avx512F.Multiply(pim48, dr));
                    Vector512<double> pre51 = Avx512F.FusedMultiplyAddNegated(pim50, di, Avx512F.Multiply(pre49, dr));
                    Vector512<double> pim52 = Avx512F.FusedMultiplyAdd(pre49, di, Avx512F.Multiply(pim50, dr));
                    Vector512<double> pre53 = Avx512F.Add(pre31, pre51);
                    Vector512<double> pim54 = Avx512F.Add(pim32, pim52);
                    Vector512<double> pre55 = Vector512.Create(0.041666666666666664);
                    Vector512<double> pre56 = Vector512.Create(5.0);
                    Vector512<double> pre57 = Vector512.Create(4.0);
                    Vector512<double> pre58 = Vector512.Create(3.0);
                    Vector512<double> pre59 = Vector512.Create(2.0);
                    Vector512<double> pre60 = Avx512F.Multiply(pre59, Zr_v);
                    Vector512<double> pim61 = Avx512F.Multiply(pre59, Zi_v);
                    Vector512<double> pre62 = Avx512F.Multiply(pre58, pre60);
                    Vector512<double> pim63 = Avx512F.Multiply(pre58, pim61);
                    Vector512<double> pre64 = Avx512F.Multiply(pre57, pre62);
                    Vector512<double> pim65 = Avx512F.Multiply(pre57, pim63);
                    Vector512<double> pre66 = Avx512F.Multiply(pre56, pre64);
                    Vector512<double> pim67 = Avx512F.Multiply(pre56, pim65);
                    Vector512<double> pre68 = Avx512F.Multiply(pre55, pre66);
                    Vector512<double> pim69 = Avx512F.Multiply(pre55, pim67);
                    Vector512<double> pre70 = Avx512F.FusedMultiplyAddNegated(pim69, di, Avx512F.Multiply(pre68, dr));
                    Vector512<double> pim71 = Avx512F.FusedMultiplyAdd(pre68, di, Avx512F.Multiply(pim69, dr));
                    Vector512<double> pre72 = Avx512F.FusedMultiplyAddNegated(pim71, di, Avx512F.Multiply(pre70, dr));
                    Vector512<double> pim73 = Avx512F.FusedMultiplyAdd(pre70, di, Avx512F.Multiply(pim71, dr));
                    Vector512<double> pre74 = Avx512F.FusedMultiplyAddNegated(pim73, di, Avx512F.Multiply(pre72, dr));
                    Vector512<double> pim75 = Avx512F.FusedMultiplyAdd(pre72, di, Avx512F.Multiply(pim73, dr));
                    Vector512<double> pre76 = Avx512F.FusedMultiplyAddNegated(pim75, di, Avx512F.Multiply(pre74, dr));
                    Vector512<double> pim77 = Avx512F.FusedMultiplyAdd(pre74, di, Avx512F.Multiply(pim75, dr));
                    Vector512<double> pre78 = Avx512F.Add(pre53, pre76);
                    Vector512<double> pim79 = Avx512F.Add(pim54, pim77);
                    Vector512<double> pre80 = Avx512F.FusedMultiplyAddNegated(di, di, Avx512F.Multiply(dr, dr));
                    Vector512<double> pim81 = Avx512F.FusedMultiplyAdd(dr, di, Avx512F.Multiply(di, dr));
                    Vector512<double> pre82 = Avx512F.FusedMultiplyAddNegated(pim81, di, Avx512F.Multiply(pre80, dr));
                    Vector512<double> pim83 = Avx512F.FusedMultiplyAdd(pre80, di, Avx512F.Multiply(pim81, dr));
                    Vector512<double> pre84 = Avx512F.FusedMultiplyAddNegated(pim83, di, Avx512F.Multiply(pre82, dr));
                    Vector512<double> pim85 = Avx512F.FusedMultiplyAdd(pre82, di, Avx512F.Multiply(pim83, dr));
                    Vector512<double> pre86 = Avx512F.FusedMultiplyAddNegated(pim85, di, Avx512F.Multiply(pre84, dr));
                    Vector512<double> pim87 = Avx512F.FusedMultiplyAdd(pre84, di, Avx512F.Multiply(pim85, dr));
                    Vector512<double> pre88 = Avx512F.Add(pre78, pre86);
                    Vector512<double> pim89 = Avx512F.Add(pim79, pim87);
                    Vector512<double> dr_new = pre88;
                    Vector512<double> di_new = pim89;
                            Vector512<double> keep = activeMask.AsDouble();
            ...
            for (; x < Width; x++)
            {
                ...
                double dr = 0.0, di = 0.0;     // δ
                {
                    ...
                    for (; it < refOrbitLen; it++)
                    {
                        ...
                        {
                    double drv_new = (((5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zr - ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zi)) * drv - (5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zi + ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zr)) * div) + 1.0);
                    double div_new = ((5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zr - ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zi)) * div + (5.0 * (((zr * zr - zi * zi) * zr - (zr * zi + zi * zr) * zi) * zi + ((zr * zr - zi * zi) * zi + (zr * zi + zi * zr) * zr) * zr)) * drv);
                    ...
                    double dr_new = (((((er + ((5.0 * (((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi) * Zr - ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr) * Zi)) * dr - (5.0 * (((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi) * Zi + ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr) * Zr)) * di)) + (((0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi)))) * dr - (0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr)))) * di) * dr - ((0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi)))) * di + (0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr)))) * dr) * di)) + ((((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * dr - (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * di) * dr - ((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * di + (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * dr) * di) * dr - (((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * dr - (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * di) * di + ((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * di + (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * dr) * dr) * di)) + (((((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * dr - ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * di) * dr - (((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * di + ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * dr) * di) * dr - ((((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * dr - ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * di) * di + (((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * di + ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * dr) * dr) * di)) + ((((dr * dr - di * di) * dr - (dr * di + di * dr) * di) * dr - ((dr * dr - di * di) * di + (dr * di + di * dr) * dr) * di) * dr - (((dr * dr - di * di) * dr - (dr * di + di * dr) * di) * di + ((dr * dr - di * di) * di + (dr * di + di * dr) * dr) * dr) * di));
                    double di_new = (((((ei + ((5.0 * (((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi) * Zr - ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr) * Zi)) * di + (5.0 * (((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi) * Zi + ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr) * Zr)) * dr)) + (((0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi)))) * dr - (0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr)))) * di) * di + ((0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zr - (Zr * Zi + Zi * Zr) * Zi)))) * di + (0.5 * (5.0 * (4.0 * ((Zr * Zr - Zi * Zi) * Zi + (Zr * Zi + Zi * Zr) * Zr)))) * dr) * dr)) + ((((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * dr - (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * di) * dr - ((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * di + (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * dr) * di) * di + (((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * dr - (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * di) * di + ((0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zr - Zi * Zi))))) * di + (0.16666666666666666 * (5.0 * (4.0 * (3.0 * (Zr * Zi + Zi * Zr))))) * dr) * dr) * dr)) + (((((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * dr - ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * di) * dr - (((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * di + ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * dr) * di) * di + ((((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * dr - ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * di) * di + (((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * dr - (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * di) * di + ((0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zr))))) * di + (0.041666666666666664 * (5.0 * (4.0 * (3.0 * (2.0 * Zi))))) * dr) * dr) * dr) * dr)) + ((((dr * dr - di * di) * dr - (dr * di + di * dr) * di) * dr - ((dr * dr - di * di) * di + (dr * di + di * dr) * dr) * di) * di + (((dr * dr - di * di) * dr - (dr * di + di * dr) * di) * di + ((dr * dr - di * di) * di + (dr * di + di * dr) * dr) * dr) * dr));
                        dr = dr_new; di = di_new;
                    }
