...
- [ ] `SilkCglContextAdapter` — NSOpenGLContext / CGLChoosePixelFormat via the ObjC runtime so macOS Avalonia (CAMetalLayer surface) can drive the Silk renderer. Required before the macOS CI leg can run the smoke under a real window server.
- [ ] `SilkEglContextAdapter` for Wayland (`eglGetDisplay(WL_DISPLAY)` + `eglCreatePlatformWindowSurfaceEXT`) so Avalonia's Wayland backend on modern Linux distros stops falling through to the X11 path under XWayland.
- [ ] Offscreen FBO smoke variant so CI can exercise the upload + draw path without spawning a window — would unblock the macOS leg and remove the `xvfb-run` dependency on Linux.
- [ ] Sibling `FracturingFog.Rendering.Skia` (SkiaSharp GPU backend) for the case where shipping native GL drivers is undesirable — Skia GPU paints the same uint[] BGRA buffer through its own backend.
