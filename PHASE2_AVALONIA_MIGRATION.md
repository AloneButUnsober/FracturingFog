...
- [x] Full FloatingHelp text migration (~2,500 lines) — extract from `Views/FloatingHelp.cs` into shared resource bundle both shells read *(partially done — `Abstractions/Help/HelpTextBundle.cs` now holds AboutText / FeaturesText / AudioText / EditorText / BioText + the 4 math sub-tabs exposed by `IHelpContentProvider` (Mandelbrot, Julia, Newton, Mandelbulb). `HostHelpContentProvider` serves directly from the bundle so the Avalonia FloatingHelp matches the WinForms long-form text. The remaining ~15 legacy-only math sub-tabs (Burning Ship / Tricorn / Multibrot / Phoenix / Nova / Buddhabrot / IFS / L-System / Attractor / User Equation / User Bulb / Sandbox) stay inline in `Views/FloatingHelp.cs` until `IHelpContentProvider` grows the surface area to expose them — tracked as task #46)*
