...
// D-3b acceptance harness: render the same image two ways and report
// wall-time + speedup. Both arms use the real HostFractalRenderEngine
// and the real TilePlanner / TileDispatcher / ArtifactMerger — they
// differ only in how many concurrent in-process "workers" pull tiles.
//
//   1. baseline   — single thread, plan + render + merge in order.
...
// path is already covered by ClusterEndToEndImageTests; what's new
// here is the parallel-execution speedup measurement called for by
// the dev plan §9 D-3 exit criteria. Output: "cluster-scale.out" next
// to the exe.
//
// Invocation:
//   FracturingFog --cluster-scale [--width N] [--height N] [--tile-px N]
...
// run a Bird-of-Paradise 8K stress.
...
public static class ClusterScaleSelfTest
{
    public static int Run(string[] args)
    {
        ...
        sb.AppendLine("cluster-scale self-test (D-3b)");
        ...
        try
        {
            long baselineMs = RunBaseline(engine, codec, baseReq, tilePx, log, ct, sb);
            long parallelMs = RunParallel(engine, codec, baseReq, tilePx, workerCount, log, ct, sb);
