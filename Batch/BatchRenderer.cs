...
namespace FracturingFog.Batch
{
    public static class BatchRenderer
    {
        ...
        internal static FractalParameters BuildFractalParameters(BatchOptions opts)
        {
            ...
            if (opts.ReliefMotionBlur.HasValue)        fp.Relief2DMotionBlurStrength = opts.ReliefMotionBlur.Value;      // S1 (#398)
            if (opts.ReliefMotionBlurSamples.HasValue) fp.Relief2DMotionBlurSamples  = opts.ReliefMotionBlurSamples.Value;
            ...
            if (opts.Transmission.HasValue || opts.Ior.HasValue || opts.AbsorptionDist.HasValue
                || opts.AbsorptionColor.HasValue || opts.GlassInternalMarch || opts.GlassInternalBounces.HasValue)
            {
                var fxg = fp.Lighting;
                ...
                if (opts.GlassInternalBounces.HasValue) fxg.RefractInternalBounces = opts.GlassInternalBounces.Value;
                fp.Lighting = fxg;
            }
        ...
        private static bool ReliefHdrWanted(
            ...
            => vt != FracturingFog.Imaging.ViewTransform.None
               ...
               // volume into the captured HDR beauty too, so froxel fog survives the tonemap
               // and HDR capture is no longer gated off under froxel.
