...
public sealed class ComplexProceduralAnimator : ProceduralAnimator
{
    ...
    public override void Tick(double dt)
    {
        ...
        if (Track.Mode == AnimationMode.CardioidApproach)
        {
            ...
            // boundary c(θ) (multiplier e^{2πiθ}). θ → p/q approaches the period-q
            // parabolic root; the near-parabolic Julia set blooms satellite
            // cascades — the Lavaurs-limit implosion.
            double theta = Track.Min + (Track.Max - Track.Min) * TriangleUnit(Phase);
            _setter(ParabolicImplosionMath.CardioidPoint(theta));
            return;
        }
